...
export async function payout(req, res) {
    ...
    // // sync block
    // try {
    // var result = await sync_block.syncHistory(txn);
    // console.log("Value returned by syncHistory to sc.payout: " + result);
    // if (result == undefined) process.exit();
    // } 
    // catch (e) {
    //     console.log("Error in value returned by syncHistory to sc payout.");
    //     throw e;
    // }

    try {
    var val = await sync_block.syncRecordPassive(txn);
    console.log("Value returned by syncRecordPassive to sc.payout: " + result);
    if (val == undefined) process.exit();
    } 
    catch (e) {
        console.log("Error in value returned by syncRecordPassive to sc payout.");
        throw e;
    }
